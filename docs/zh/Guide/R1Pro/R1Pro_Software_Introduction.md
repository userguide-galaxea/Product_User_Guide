...
下图为力位混合控制模式架构：</br>
![R1Pro_arm_motor_cn](D:\星海图\2_用户手册\R1 Pro\R1Pro_软件介绍\R1Pro_arm_motor_cn.png)
...
#### 底盘控制
R1 Pro底盘控制是一个使用矢量控制来控制R1 Pro底盘的节点，它允许您同时发送三个方向的速度命令：x、y 和w。
...
这个启动文件将启动两个节点：`chassis_control_node` 和 `r1_pro_eepose_pub_node`。`chassis_control_node `为R1 Pro底盘的速度控制。其接口如下所示：
<table style="width: 100%; border-collapse: collapse;">
  <thead>
    <tr>
      <th style="background-color: black; color: white; vertical-align: middle; padding: 8px; border: 1px solid #ddd; width: 200px;">话题名称</th>
