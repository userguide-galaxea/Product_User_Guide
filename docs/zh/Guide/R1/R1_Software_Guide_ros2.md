...
![R1_software_structure_cn](assets/R1_software_structure_cn.png)
...
#### 底盘驱动接口
该接口是用于底盘状态反馈的ROS软件包，定义了多个话题以报告底盘电机的状态。接口信息如下：
<table style="width: 100%; border-collapse: collapse;table-layout: fixed;">
    <thead>
        <tr style="background-color: black; color: white; text-align: left;">
...
#### 关节控制
R1关节控制节点负责控制R1躯干和手臂的每个关节，总共有16个关节。可通过以下命令启动：
...
该文件将启动 `r1_jointTracker_demo_node`，该节点是负责控制每个关节的主要节点。接口信息如下：
<table style="width: 100%; border-collapse: collapse;">
  <thead>
    <tr>
...
#### 手臂姿态控制
R1手臂姿态控制是一个用于控制手臂移动到目标末端执行器（ee）坐标帧的ROS软件包。它主要包括两个launch文件,分别对应 左臂的姿态控制和右臂的姿态控制, 可通过以下命令启动：
...
R1躯干速度控制是一个用于控制躯干移动到目标浮动基座坐标帧的ROS软件包。可通过以下命令启动：
...
#### 底盘控制
底盘控制是一个矢量控制节点，允许同时发送三个方向的速度命令：x、y 和w。可通过以下命令启动：
